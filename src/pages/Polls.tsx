...
export default function PollsPage() {
  ...
  const pollsQuery = useQuery({
    ...
    queryFn: () => {
      ...
      return api.get(`/polls${params}`).then((data) => data.data ?? data)
    },
  })
  ...
  const deleteMutation = useMutation({
    mutationFn: (pollId: number) => api.delete(`/polls/${pollId}`),
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['polls'] }),
  })
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label htmlFor="poll-ends" className="text-sm font-bold">
                  Дата завершения
                </label>
                <Input
                  id="poll-ends"
              ...
              <div className="space-y-1.5">
                <label htmlFor="poll-results" className="text-sm font-bold">
                  Показ результатов
                </label>
                <Select value={form.showResults} onValueChange={(v) => setForm({ ...form, showResults: v })}>
                  <SelectTrigger id="poll-results" className="w-full">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
            ...
            <div className="flex gap-2">
              <Button
                ...
                disabled={
                  !form.title.trim() || form.options.filter((o) => o.trim()).length < 2 || createMutation.isPending
                }
              >
                {t('polls.submitBtn')}
              </Button>
              ...
              <Card key={poll.id} className="flex flex-col">
                <CardContent className="p-5 flex-1 flex flex-col">
                      ...
                      {poll.options.map((opt) => {
                        ...
                        return (
                          <div
                            key={opt.id}
                            onClick={() => {
                              if (!poll.isClosed) voteMutation.mutate({ pollId: poll.id, optionId: opt.id })
                            }}
                          ...
                          >
                            {showResultsNow && (
                              <div
                                className="absolute inset-y-0 left-0 bg-primary/5 transition-all rounded-lg"
                                style={{ width: `${pct}%` }}
                              />
                            )}
                            <div className="relative z-10 flex items-center justify-between gap-2">
                              <div className="flex items-center gap-1.5 min-w-0">
                                {showResultsNow && isSelected && (
                  ...
                  <div className="flex items-center justify-between mt-4 pt-3 border-t text-[10px] text-muted-foreground">
                    <div className="flex items-center gap-2">
                      <span>
                        {poll.options.length} вар. · {poll.totalVotes} голосов
                      </span>
                    ...
                    <div className="flex items-center gap-1">
                      {hasVoted ? (
                        <Badge variant="secondary" className="text-[9px] gap-0.5">
                          <CheckCheck className="w-2.5 h-2.5" /> Голос учтён
                        </Badge>
                      ) : !poll.isClosed ? (
                      ...
                      {(canManage || user?.id === poll.createdBy) && (
                        <Button
                          variant="ghost"
                          size="icon"
                          className="w-6 h-6 text-destructive/60 hover:text-destructive"
